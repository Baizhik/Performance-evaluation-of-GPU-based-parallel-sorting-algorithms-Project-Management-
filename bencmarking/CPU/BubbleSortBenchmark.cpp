

#include <algorithm>
#include <atomic>
#include <chrono>
#include <cmath>
#include <fstream>
#include <iomanip>
#include <iostream>
#include <numeric>
#include <stdexcept>
#include <string>
#include <vector>

using namespace std;
using namespace std::chrono;


static atomic<long long> g_live{0};
static atomic<long long> g_peak{0};

namespace {
struct AllocHeader { size_t sz; };

inline void* alloc_and_track(size_t sz) {
    void* raw = std::malloc(sizeof(AllocHeader) + sz);
    if (!raw) throw std::bad_alloc();
    static_cast<AllocHeader*>(raw)->sz = sz;
    void* p = static_cast<char*>(raw) + sizeof(AllocHeader);

    long long now = (g_live += (long long)sz);
    long long prev = g_peak.load(std::memory_order_relaxed);
    while (now > prev && !g_peak.compare_exchange_weak(
               prev, now, std::memory_order_relaxed, std::memory_order_relaxed)) {}
    return p;
}
inline void free_and_track(void* p) noexcept {
    if (!p) return;
    void* raw = static_cast<char*>(p) - sizeof(AllocHeader);
    size_t sz = static_cast<AllocHeader*>(raw)->sz;
    g_live -= (long long)sz;
    std::free(raw);
}
}

void* operator new(std::size_t sz)             { return alloc_and_track(sz); }
void* operator new[](std::size_t sz)           { return alloc_and_track(sz); }
void  operator delete(void* p) noexcept        { free_and_track(p); }
void  operator delete[](void* p) noexcept      { free_and_track(p); }
void  operator delete(void* p, std::size_t) noexcept   { free_and_track(p); }
void  operator delete[](void* p, std::size_t) noexcept { free_and_track(p); }

static inline void reset_peak()         { g_live.store(0); g_peak.store(0); }
static inline long long peak_bytes()    { return g_peak.load(); }


static const int RUNS = 30;

const string INPUT_FILE     = "C:/Users/user/Desktop/CPU/data/sorted_10e6.csv";
const string SORTED_FILE    = "C:/Users/user/Desktop/CPU/data/outputs/output_bubblesort_sorted.csv";
const string BENCHMARK_FILE = "C:/Users/user/Desktop/CPU/data/benchmark_results/benchmark_bubblesort.csv";


static vector<int> readCSV(const string& path) {
    ifstream in(path, ios::binary);
    if (!in) throw runtime_error("Failed to open input file: " + path);

   
    in.seekg(0, ios::end);
    size_t sz = (size_t)in.tellg();
    in.seekg(0, ios::beg);
    vector<char> buf(sz);
    in.read(buf.data(), sz);

    vector<int> a;
    a.reserve(10'000'000); 

    long sign = 1;
    long val = 0;
    bool in_num = false;

    for (size_t i = 0; i < sz; ++i) {
        char c = buf[i];
        if (c == '-') { sign = -1; in_num = true; val = 0; }
        else if (c >= '0' && c <= '9') { in_num = true; val = val*10 + (c - '0'); }
        else {
            if (in_num) { a.push_back((int)(sign * val)); sign = 1; val = 0; in_num = false; }
        }
    }
    if (in_num) a.push_back((int)(sign * val)); 

    return a;
}

static void writeCSV(const string& path, const vector<int>& v) {
    ofstream out(path);
    for (int x : v) out << x << "\n";
}
static void writeBenchmarkCSV(const string& path,
                              const vector<long long>& dur_ms,
                              const vector<long long>& mem_bytes) {
    ofstream out(path);
    out << "Run,Duration(ms),Memory(MB)\n";
    out << fixed << setprecision(2);
    for (size_t i = 0; i < dur_ms.size(); ++i)
        out << (i+1) << "," << dur_ms[i] << "," << (double(mem_bytes[i]) / 1e6) << "\n";
}
static double avg(const vector<long long>& v){
    if (v.empty()) return 0.0;
    long double s=0; for (auto x:v) s+=x;
    return (double)(s/v.size());
}
static double stdev(const vector<long long>& v, double mean){
    if (v.empty()) return 0.0;
    long double s=0; for (auto x:v){ long double d=x-mean; s+=d*d; }
    return (double)std::sqrt(s/v.size());
}


static inline void iswap(int& a, int& b) { int t = a; a = b; b = t; }

void bubbleSortBaseline(vector<int>& a) {
    const int n = (int)a.size();
    if (n <= 1) return;

    for (int pass = 0; pass < n - 1; ++pass) {
        for (int i = 0; i < n - 1 - pass; ++i) {
            if (a[i] > a[i + 1]) {
                int temp = a[i];
                a[i] = a[i + 1];
                a[i + 1] = temp;
            }
        }
    }
}




int main() {
    ios::sync_with_stdio(false);
    cin.tie(nullptr);

    auto t_io0 = std::chrono::high_resolution_clock::now();
    std::vector<int> original = readCSV(INPUT_FILE); 
    auto t_io1 = std::chrono::high_resolution_clock::now();

    std::cout << "Loaded " << original.size() << " ints in "
              << std::chrono::duration_cast<std::chrono::seconds>(t_io1 - t_io0).count()
              << " s\n";

    std::vector<long long> durations_ms(RUNS);
    std::vector<long long> peak_mem(RUNS);

    for (int run = 0; run < RUNS; ++run) {
        reset_peak();
        std::vector<int> arr = original; 

        auto t0 = std::chrono::high_resolution_clock::now();
        bubbleSort(arr);  
        auto t1 = std::chrono::high_resolution_clock::now();

        long long ms = std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count();
        durations_ms[run] = ms;
        peak_mem[run] = peak_bytes();

        std::cout << "Run " << (run + 1) << ": " << ms
                  << " ms | Peak memory: " << std::fixed << std::setprecision(2)
                  << (double(peak_mem[run]) / 1e6) << " MB\n";
    }

    double aT   = avg(durations_ms);
    double sT   = stdev(durations_ms, aT);
    double sPct = (aT == 0.0 ? 0.0 : (sT / aT) * 100.0);
    long long mnT = *std::min_element(durations_ms.begin(), durations_ms.end());
    long long mxT = *std::max_element(durations_ms.begin(), durations_ms.end());

    double aM  = avg(peak_mem) / 1e6;
    double mnM = (*std::min_element(peak_mem.begin(), peak_mem.end())) / 1e6;
    double mxM = (*std::max_element(peak_mem.begin(), peak_mem.end())) / 1e6;

    std::string interp = (sPct < 5.0) ? " Very stable performance"
                        : (sPct <= 15.0) ? " Fairly stable performance"
                        : " Some variation in performance";

    std::cout << "\n Benchmark Summary (" << RUNS << " runs)\n";
    std::cout << "  Average time: " << std::fixed << std::setprecision(2) << aT << " ms\n";
    std::cout << "  Std deviation: " << sT << " ms (" << sPct << "%)\n";
    std::cout << "  Min: " << mnT << " ms | Max: " << mxT << " ms\n";
    std::cout << "  Interpretation: " << interp << "\n";
    std::cout << " Peak memory (avg): " << aM
              << " MB | min: " << mnM << " MB | max: " << mxM << " MB\n";

    writeBenchmarkCSV(BENCHMARK_FILE, durations_ms, peak_mem);
    std::cout << " Benchmark report saved: " << BENCHMARK_FILE << "\n";

    std::vector<int> sorted = original;
    bubbleSort(sorted);  // <-- same function here
    writeCSV(SORTED_FILE, sorted);
    std::cout << " Sorted file saved: " << SORTED_FILE << "\n";

    return 0;
}

